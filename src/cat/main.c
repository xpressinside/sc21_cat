...
void CatReadPrintFile(FILE *file, Flags flags, int *linenumber, int *last,
                      bool *firstLine, bool *squeeze);
...
Flags CatReadFlags(int argc, char *argv[]) {
  ...
  int currentFlag;
  while ((currentFlag = getopt_long(argc, argv, "bevEnstT", longFlags, NULL)) !=
         -1) {
    switch (currentFlag) {
      case 'b':
